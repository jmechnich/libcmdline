...
**    $RCSfile: CmdLine.cc,v $
**   $Revision: 1.16 $$Name:  $
**       $Date: 2005/06/09 07:52:11 $
         ...
         pUsageXMLArg(0, "usagexml", "Display XML usage message.")
{
  ...
  va_list ap;
  va_start(ap, cmdarg1);
  for (CmdArg* cmdarg=cmdarg1; 
       cmdarg!=0; 
       cmdarg=va_arg(ap, CmdArg*)) 
  {
    append(cmdarg);
  }
  va_end(ap);
    ...
    std::ostream& os) const
{
  ...
  unsigned int curPos=pName.size()+7;
       ...
       positionals++)
  {
         ...
         ++itL)
    {
      ...
      buf=cmdarg->synopsis(
//          true,
          syntax==USAGE_SHORT_ONLY || syntax==USAGE_BOTH,
          syntax==USAGE_LONG_ONLY || syntax==USAGE_BOTH);
      len=pl=buf.size();
...
basename(const std::string& filename)
{
  ...
  unsigned long slash=filename.rfind('/');
    ...
    const std::string& replacedBy)
{
  ...
  for (unsigned long i=s.find(replacedText);
       i!=std::string::npos;
